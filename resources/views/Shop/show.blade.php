        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
                ...
                <div class="flex flex-col justify-center">
                    <div class="mb-4">
                        <span class="bg-pink-100 text-pink-600 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">
                            {{ $product->category->name ?? 'Skincare' }}
                        </span>
                    </div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">{{ $product->name }}</h1>
                    <p class="text-2xl font-bold text-pink-600 mb-6">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                    ...
                    <div class="flex items-center gap-3 mb-6 border-b border-gray-100 pb-6">
                        <div class="w-10 h-10 bg-gray-200 rounded-full overflow-hidden">
                            <img src="{{ $product->store->logo_path ? asset('storage/' . $product->store->logo_path) : 'https://via.placeholder.com/40' }}" class="w-full h-full object-cover">
                        </div>
                        <div>
                    ...
                    <div class="flex gap-4">
                        <form action="{{ route('cart.add', $product->id) }}" method="POST" class="flex-1">
                            @csrf
                            <button class="w-full bg-gray-900 text-white py-4 rounded-xl font-bold hover:bg-pink-600 transition shadow-xl">
                                Add to Cart
                            </button>
                        </form>
                        <form action="{{ route('wishlist.toggle', $product->id) }}" method="POST">
                            @csrf
                            <button class="bg-white border-2 border-gray-200 text-gray-400 p-4 rounded-xl hover:border-pink-500 hover:text-pink-500 transition">
                                ♥ Wishlist
                            </button>
                        </form>
                    </div>
